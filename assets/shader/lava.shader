...
float rand(vec2 coord){
	return fract(sin(dot(coord, vec2(2.456456456456546546456456, 149.45474545645655465))));
}
...
void fragment(){
	vec2 coord = UV *1.0;
	float noise1 = fbm(coord + vec2(TIME * -0.125, TIME * 0.25));
	float noise2 = fbm(coord + vec2(TIME * 0.125, TIME * -0.125));
	float noise3 = fbm(coord + vec2(TIME * 0.25, -TIME * 0.25));
	float combined = noise1 * noise2 * noise3 * 3.0;
	vec4 color = vec4(vec3(combined),1.0);
	COLOR = color;
