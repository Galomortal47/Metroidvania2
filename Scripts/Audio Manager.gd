...
func _process(delta):
	...
	if Input.is_action_pressed("ui_roll") and not int(get_parent().motion.x) == 0 and get_parent().ground_detect():
		$Roll.set_volume_db(0)
	else:
		$Roll.set_volume_db(-50)
	if Input.is_action_pressed("ui_accept") and get_parent().ground_detect():
